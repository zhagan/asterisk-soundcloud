...
while selection==False:
	try:
 		...
 		genre = agi.get_variable("genre")
   		...
   		users = client.get('/users', q=searchVar, limit=page_size, offset=page_size*offset)
  		...
  		#maybe I should break this up but there is a lot of formatting going on
   			answerStr = agi.appexec('Flite', 'press '+str(i)+' for '+u.full_name.encode('utf-8').translate(None, '"',)+' '+u.description.encode('utf-8').translate(None, '"',)+',any')
  			...
  			if answer == -7:
				#selection = False
  				...
  				selection = True
  				break
   		 	agi.verbose(u.full_name.encode('utf-8'))
   			i =i+1
   ...
   #my_var = agi.get_variable('MyVar')
   		...
   		agitb.handler()
   		sys.exit(1)
userid = users[answer].id  
agi.set_variable("userid", userid)
